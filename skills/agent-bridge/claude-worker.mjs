...
const o=args(process.argv.slice(2));const hasAgent=!!(o.agent||o.agentFile);if(hasAgent===!!o.skill)fail(2,'ERROR','Pass exactly one anchor: --agent/--agent-file or --skill');if(!o.model)fail(2,'ERROR','Claude Code worker requires an explicit model from its profile');if(!['auto','yes','no'].includes(o.readonly))fail(2,'ERROR','--readonly must be auto, yes, or no');if(o.maxTurns!==undefined&&(!Number.isInteger(o.maxTurns)||o.maxTurns<1))fail(2,'ERROR','--max-turns must be positive');if(o.needsBrowser)fail(3,'BLOCKED','Claude Code external worker browser capability is not verified in worker-profile v1; route this role to a verified browser profile instead.');
...
const info={status:'OK',agent:name,agentFile,sdk:'claude-code',profile:process.env.AGENT_BRIDGE_PROFILE||null,workerConfig:process.env.AGENT_BRIDGE_WORKER_CONFIG_RESOLVED||null,model:o.model,effort:o.effort||null,readonly:ro,needsBrowser:false,skills:skills.map(s=>s.name),promptBytes:Buffer.byteLength(prompt),launcherKind:launch.kind};
