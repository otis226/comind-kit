...
if (!fs.existsSync(skillsRoot)) {
  ...
  for (const entry of fs.readdirSync(skillsRoot, { withFileTypes: true })) {
    ...
    if (!fs.existsSync(skillFile)) {
      errors.push(`Skill package missing SKILL.md: ${entry.name}`);
      continue;
    }
    ...
    if (!nameMatch || nameMatch[1].trim() !== entry.name) {
      errors.push(`Skill frontmatter name does not match directory: ${entry.name}`);
    }
    if (skillText.includes('INSTRUCTIONS.md') && !fs.existsSync(path.join(skillsRoot, entry.name, 'INSTRUCTIONS.md'))) {
      errors.push(`SKILL.md references missing INSTRUCTIONS.md: ${entry.name}`);
    }
  }
}
...
const runtimeForbidden = [
  [/skills\/[a-z0-9-]+\.md\b/i, 'legacy flat skill path'],
];
...
for (const file of walk(root)) {
  ...
  for (const [pattern, label] of globalForbidden) {
    if (pattern.test(text)) errors.push(`${relative}: ${label}`);
  }

  const isRuntimeInstruction = relative.startsWith('skills/') || relative.startsWith('agents/');
  if (isRuntimeInstruction) {
    for (const [pattern, label] of runtimeForbidden) {
      if (pattern.test(text)) errors.push(`${relative}: ${label}`);
    }
    if (relative.endsWith('.md') && vietnameseSignals.test(text)) {
      errors.push(`${relative}: reusable runtime instructions should be English-first`);
    }
  }
}
