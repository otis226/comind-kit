...
import { resolveBin, spawnHeadless } from './process-launch.mjs';

const HOME=os.homedir();
function fail(code,status,error,extra={}){process.stdout.write(JSON.stringify({status,agent:null,sdk:'claude-code',...extra,error},null,2)+'\n');process.exit(code)}
...
if(process.env.AGENT_BRIDGE_EXTERNAL!=='1')fail(2,'ERROR','Invoke external Claude Code through dispatch.mjs so explicit credential isolation is enforced.');
const o=args(process.argv.slice(2));const hasAgent=!!(o.agent||o.agentFile);if(hasAgent===!!o.skill)fail(2,'ERROR','Pass exactly one anchor: --agent/--agent-file or --skill');if(!o.model)fail(2,'ERROR','External Claude Code worker requires an explicit --model');if(!['auto','yes','no'].includes(o.readonly))fail(2,'ERROR','--readonly must be auto, yes, or no');if(o.maxTurns!==undefined&&(!Number.isInteger(o.maxTurns)||o.maxTurns<1))fail(2,'ERROR','--max-turns must be positive');if(o.needsBrowser)fail(3,'BLOCKED','External Claude Code browser capability is not verified; use a runtime with verified browser capability.');
const launch=resolveBin('claude');if(!launch)fail(3,'BLOCKED','Claude Code CLI "claude" is not on PATH.');if(!launch.command)fail(3,'BLOCKED',launch.error||'Claude Code CLI launcher cannot be executed safely on this platform.');
